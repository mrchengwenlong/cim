...
public class MsgHandler implements MsgHandle {
    ...
    private void normalChat(String msg) {
        ...
        String[] totalStoreMsg=msg.split(";");
        ...
        } else  if (totalStoreMsg.length>1) {
            //离线私聊
            LOGGER.info("p2pStore");
            P2PReqVO p2PReqVO = new P2PReqVO();
            p2PReqVO.setUserId(configuration.getUserId());
            p2PReqVO.setReceiveUserId(Long.parseLong(totalStoreMsg[0]));
            p2PReqVO.setMsg(totalStoreMsg[1]);
            try {
                p2pStoreChat(p2PReqVO);
            } catch (Exception e) {
                LOGGER.error("Exception", e);
            }
        }else {
    ...
    @Override
    public void p2pStoreChat(P2PReqVO p2PReqVO) throws Exception {
        routeRequest.SendP2PStoreMsg(p2PReqVO);
    }
